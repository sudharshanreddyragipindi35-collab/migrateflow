...
export function ReviewQueue({ batchId, onCompleted }: { batchId: string; onCompleted: () => void }) {
  ...
  return <section>
    ...
    <div className="card-grid">{items.map((item) => {
      ...
      const isDateCorrection = recordReview && context.field?.endsWith("_date");
      return <article className="review-card" key={item.escalation_id}>
        <div className="card-top"><span>{context.source_file}{context.source_record_id ? ` - row ${context.source_record_id}` : ""}</span><strong>{item.reason_code.replaceAll("_", " ")}</strong></div>
        <h3>{recordReview ? context.field : context.source_column}</h3>
        <p>{recordReview ? "Current value" : "Recommended target"}: <code>{String(context.current_value ?? item.suggestion ?? "Not available")}</code></p>
        {context.errors?.length ? <ul className="review-errors">{context.errors.map((message) => <li key={message}>{message}</li>)}</ul> : null}
        <div className="confidence">{Object.entries(item.confidence_evidence).map(([key, value]) => <label key={key}><span>{key.replaceAll("_", " ")}</span><progress max="1" value={value} /><small>{Math.round(value * 100)}%</small></label>)}</div>
        {can("CORRECT") && <label>Correction
          {recordReview && !alternatives.length
            ? <><input type={isDateCorrection ? "date" : "text"} value={correction[item.escalation_id] ?? ""} onChange={(event) => setCorrection({ ...correction, [item.escalation_id]: event.target.value })} placeholder={`Enter corrected ${context.field ?? "value"}`} />{isDateCorrection && <small>Use YYYY-MM-DD. If the correct date is unknown, reject the record instead of guessing.</small>}</>
            : <select value={correction[item.escalation_id] ?? ""} onChange={(event) => setCorrection({ ...correction, [item.escalation_id]: event.target.value })}><option value="">Select a correction</option>{correctionOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select>}
        </label>}
        <div className="actions">{can("REJECT") && <button onClick={() => decide(item, "REJECT")}>Reject</button>}{can("CORRECT") && <button onClick={() => decide(item, "CORRECT")}>Correct</button>}{can("APPROVE") && <button className="primary" onClick={() => decide(item, "APPROVE")}>Approve</button>}</div>
      </article>;
    })}</div>
  </section>;
}
