...
export function App() {
  ...
  return <div className="shell"><aside className="sidebar"><a className="brand" href="#main" aria-label="MigrateFlow home"><span>M</span><strong>MigrateFlow</strong></a><nav aria-label="Primary navigation">{navigation.map(([id, icon, label]) => <button key={id} className={view === id ? "active" : ""} aria-current={view === id ? "page" : undefined} onClick={() => setView(id)}><span>{icon}</span>{label}{id === "review" && <b>!</b>}</button>)}</nav><div className="sidebar-foot"><span className="secure-dot" />Secure model path<p>PII-safe context</p></div></aside><main id="main"><header><div><span className="secure-dot" /> Supervised mode</div>{batchId ? <code>{batchId.slice(0, 8)}</code> : <span>No active batch</span>}</header><div className="content">{content}</div></main></div>;
}
